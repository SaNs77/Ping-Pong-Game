...
function render() {
  ...
  if (user.score == 3 || com.score == 3) {
    ...
    drawRect(0, 0, canvas.width, canvas.height, "#000");
    // draw the user score to the left
    drawText(user.score, (0.8 * canvas.width) / 4, canvas.height / 4);

    drawText(com.score, (3 * canvas.width) / 4, canvas.height / 4);
    if (user.score > com.score) {
      drawText(user.gameOver, (0.7 * canvas.width) / 4, canvas.height / 2);
      yay.play();
    } else {
      drawText(com.gameOver, (0.7 * canvas.width) / 4, canvas.height / 2);
      no.play();
    }
  }
}
